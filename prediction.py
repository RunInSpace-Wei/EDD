...
class Predictor:
    ...
    def get_score(self, values):
        ...
        for i in range(preds.shape[1]):
            ...
            a_score = np.sqrt((preds[:, i] - actual[:, i]) ** 2) + self.gamma * np.sqrt(
                (recons[:, i] - actual[:, i]) ** 2)
            # pre_l = np.sqrt((preds[:, i] - actual[:, i]) ** 2)
            # rec_l = np.sqrt((recons[:, i] - actual[:, i]) ** 2)
            # a_score = pre_l + rec_l + np.abs(pre_l - rec_l)
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True,
        ...
        if true_anomalies is not None:
            ...
            bf_eval = {}
        bf_eval1 = bf_search1(test_anomaly_scores, true_anomalies, start=0.01, end=2, step_num=100, verbose=False)
        ...
        print(f"Results using best f1_1 score search:\n {bf_eval1}")
