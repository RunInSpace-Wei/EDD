...
def adjust_predicts2(score, label, threshold, pred=None, calc_latency=False):
    """
    Calculate adjusted predict labels using given `score`, `threshold` (or given `pred`) and `label`.
    Args:
            score (np.ndarray): The anomaly score
    ...
    for i in range(len(predict)):
        if predict[i] and (sum(window_label) > 0.4 * window_size):
            processed_predict.append(True)
        else:
            processed_predict.append(False)
        if len(window_label) >= window_size:
            window_label.pop(0)
        window_label.append(predict[i])
    predict = np.array(processed_predict)
...
def bf_search(score, label, start, end=None, step_num=1, display_freq=1, verbose=True):
    ...
    m_t = 0.0
    m_l = 0
    for i in tqdm(range(search_step), desc="best_f searching: "):
        threshold += search_range / float(search_step)
        target, latency = calc_seq(score, label, threshold)
        # target, latency = calc_seq1(score, label, threshold)
        if target[0] > m[0]:
            m_t = threshold
            m = target
            m_l = latency
