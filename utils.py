...
def normalize_data1(data, scaler=None):
    data = np.asarray(data, dtype=np.float32)
    ...
    if scaler is None:
        scaler = MinMaxScaler()
        scaler.fit(data.reshape(len(data), 1))
...
def get_data_dim(dataset):
    ...
    if dataset == "SMAP":
        ...
        return 38
    elif str(dataset).startswith("ETL"):
        return 12
    elif str(dataset).startswith("XSJ"):
        return 4
    elif str(dataset).startswith("SWaT"):
        return 51
    else:
        return None
...
def get_data(dataset, max_train_size=None, max_test_size=None,
    ...
    train_data = pickle.load(f)[train_start:train_end, :]
    f.close()
    try:
        f = open(os.path.join(prefix, dataset + "_test.pkl"), "rb")
        # test_data = pickle.load(f).reshape((-1, x_dim))[test_start:test_end, :]
        test_data = pickle.load(f)[test_start:test_end, :]
        f.close()
...
def get_SMD_data():
    prefix = "datasets/ServerMachineDataset/processed"
    train_data, test_data,  test_label = [], [], []
    index_id = ['1-1', '2-1', '3-2', '3-7']
    for index in index_id:
        dataset = f"machine-{index}"
        try:
            f = open(os.path.join(prefix, dataset + "_train.pkl"), "rb")
    ...
    train_data, test_data,  test_label = np.concatenate(train_data), np.concatenate(test_data), np.concatenate(test_label)
    train_data, scaler = normalize_data(train_data, scaler=None)
    test_data, _ = normalize_data(test_data, scaler=scaler)
    print("train set shape: ", train_data.shape)
    print("test set shape: ", test_data.shape)
...
def sample_anomaly_data(dataset, label, rate=0.1):
    indices = np.where(label == 1)[0]
    # 计算要抽取的异常数据数量
    n = int(rate * len(indices))
    # 随机选择要抽取的异常数据的索引
    random_indices = np.random.choice(indices, n, replace=False)
    sampler = SubsetRandomSampler(random_indices)
    dataloader = DataLoader(dataset, batch_size=1, sampler=sampler)
    abnormal_data = []
    for x, y in dataloader:
        abnormal_data.append(x)

    return abnormal_data, random_indices


def sample_normal_data(dataset, rate=0.1):
    n = int(rate * len(dataset))
    # 随机选择要抽取的异常数据的索引
    random_indices = np.random.choice(range(n), n, replace=False)
    sampler = SubsetRandomSampler(random_indices)
    dataloader = DataLoader(dataset, batch_size=1, sampler=sampler)
    normal_data = []
    for x, y in dataloader:
...
def remove_sampled_data(loss, label, sample_index):
    loss = np.delete(loss, sample_index)
    label = np.delete(label, sample_index)
    return loss, label


def kl_divergence(mu, sigma, mu_0, sigma_0):
    # 计算方差的平方
    var = sigma.pow(2)
    var_0 = sigma_0.pow(2)

    # 计算KL散度
    kld = 0.5 * (torch.log(var_0 / var) + (var + (mu - mu_0).pow(2)) / var_0 - 1)
