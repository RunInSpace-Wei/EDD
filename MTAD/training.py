    ...
    def fit(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            for x, y in train_loader:
                ...
                # forecast_loss = torch.sqrt(self.forecast_criterion(x[:, -1, :], preds))
                # recon_loss = torch.sqrt(self.recon_criterion(x, recons))

                n = epoch + 1
                loss = (1 / n) * forecast_loss + (1 - 1 / n) * recon_loss
                # loss = (1 / n) * (forecast_loss + recon_loss) + (1 - 1 / n) * torch.abs(forecast_loss - recon_loss)
